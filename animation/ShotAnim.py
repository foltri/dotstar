...
class ShotAnim(Animation):
    ...
    def animate(self):
        ...
        if not self.progress:
            ...
            self.root.STRIP.set_color_range(start_pos, start_pos + 2 * radius, Strip.DEFAULT_COLOR)

            self.root.STRIP.strip.show()
            return
        # flash
        ...
        if self.progress <= 50 + 20 + 20:
            self.root.STRIP.set_brightness(255)
            return
        ...
        if 50 + 20 + 20 + 100 < self.progress:
            ...
            self.root.STRIP.set_color_range(start_pos, start_pos + cnt, self.BLOOD_COLOR)
            self.root.STRIP.strip.show()
            return
        if self.progress <= 50 + 20 + 20 + 100 + radius * 5 + 400:
            cnt = self.progress - (50 + 20 + 20 + 100)
            rng = radius * min(1, cnt / 5)
            self.root.STRIP.set_color_range(start_pos + radius - rng, start_pos + radius + rng, self.BLOOD_COLOR)
            self.root.STRIP.strip.show()
            return
        ...
        if self.progress <= 50 + 20 + 20 + 100 + radius * 5 + 400 + radius * 90:
            ...
            self.root.STRIP.set_color_range(start_pos + radius - rng, start_pos + radius + rng, 0x0000ff)
